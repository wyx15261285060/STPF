...
public class AccountController {
    ...
    public Result<Account> login(@RequestBody Account account, HttpServletRequest request) {
        //调用isBlank方法来判断获取的姓名、密码、权限是否为null
        if (StrUtil.isBlank(account.getName()) || StrUtil.isBlank(account.getPassword()) || account.getLevel() == null) {
            throw new CustomException(ResultCode.PARAM_LOST_ERROR);
        }//声明一个level，通过accunt对象的getName()、getpassword()和getLevel()方法来
        Integer level = account.getLevel();
        Account login = new Account();
		if (1 == level) {login = adminInfoService.login(account.getName(), account.getPassword());}
		if (2 == level) {login = userInfoService.login(account.getName(), account.getPassword());}
    ...
    public Result<Account> register(@RequestBody Account account) {
        Integer level = account.getLevel();
        Account login = new Account();
		if (1 == level) {//权限为管理员时，调用copyProperties方法将注册信息拷贝到account对象
			AdminInfo info = new AdminInfo();//通过调用add方法将输入的信息添加到管理员用户信息中
			BeanUtils.copyProperties(account, info);
            login = adminInfoService.add(info);
		}
		if (2 == level) {UserInfo info = new UserInfo();BeanUtils.copyProperties(account, info);login = userInfoService.add(info);
		}
